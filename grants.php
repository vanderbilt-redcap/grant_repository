...
foreach ($awards as $award => $awardTitle) {
	if (isset($_GET[$award]) && $_GET[$award]) {
		...
		$awardField = $award;
		$awardClause = "LEFT JOIN redcap_data d7 ON (d7.project_id =d.project_id AND d7.record = d.record AND d7.field_name = '$awardField' AND d7.value='$awardValue')";
	...
	<body>
		...
		<div id="container">
			<div id="header">
				...
				<i>You may download grant documents by clicking hypertext links below. The use of the grants document repository is strictly limited to authorized individuals and you are not permitted to share files or any embedded content with other individuals. All file downloads are logged.</i>
				<hr/>
			</div>
